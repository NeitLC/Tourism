...
class FlightSearchContainer extends React.Component {
    ...
    validateForm = () => {
        if (this.state.departureAirport.code === '' || this.state.arrivalAirport.code === '') {
            this.setErrorMessage('Пожалуйста выберите аэропорты вылета и прилета из списка');
            return false;
        }

        if (!this.state.departureDate) {
            this.setErrorMessage('Пожалуйста выберите дату вылета');
            return false;
        }

        if (this.state.isRoundTrip && !this.state.returnDate) {
            this.setErrorMessage('Пожалуйста выберите дату возвращения');
            return false;
        }
